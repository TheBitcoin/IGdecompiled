...
public class ConcatenatingMediaSource extends CompositeMediaSource<MediaSourceHolder> implements PlayerMessage.Target {
    ...
    private static final class ConcatenatedTimeline extends AbstractConcatenatedTimeline {
        ...
        public ConcatenatedTimeline(Collection<MediaSourceHolder> collection, int i4, int i5, ShuffleOrder shuffleOrder, boolean z4) {
            ...
            for (MediaSourceHolder next : collection) {
                this.timelines[i6] = next.timeline;
                this.firstPeriodInChildIndices[i6] = next.firstPeriodIndexInChild;
                this.firstWindowInChildIndices[i6] = next.firstWindowIndexInChild;
        ...
        public final int getChildIndexByPeriodIndex(int i4) {
            return Util.binarySearchFloor(this.firstPeriodInChildIndices, i4 + 1, false, false);
        }

        /* access modifiers changed from: protected */
        public final int getChildIndexByWindowIndex(int i4) {
            return Util.binarySearchFloor(this.firstWindowInChildIndices, i4 + 1, false, false);
        }
    ...
    private static final class DeferredTimeline extends ForwardingTimeline {
        ...
        public final Timeline.Period getPeriod(int i4, Timeline.Period period2, boolean z4) {
            this.timeline.getPeriod(i4, period2, z4);
    ...
    private static final class DummyTimeline extends Timeline {
        ...
        public final Timeline.Window getWindow(int i4, Timeline.Window window, boolean z4, long j4) {
            ...
            return window.set((Object) null, C.TIME_UNSET, C.TIME_UNSET, false, true, j5, C.TIME_UNSET, 0, 0, 0);
        }
    ...
    static final class MediaSourceHolder implements Comparable<MediaSourceHolder> {
        ...
        public final void reset(int i4, int i5, int i6) {
            this.childIndex = i4;
            this.firstWindowIndexInChild = i5;
            this.firstPeriodIndexInChild = i6;
    ...
    private void addMediaSourceInternal(int i4, MediaSourceHolder mediaSourceHolder) {
        if (i4 > 0) {
            ...
            mediaSourceHolder.reset(i4, mediaSourceHolder2.firstWindowIndexInChild + mediaSourceHolder2.timeline.getWindowCount(), mediaSourceHolder2.firstPeriodIndexInChild + mediaSourceHolder2.timeline.getPeriodCount());
        } else {
            mediaSourceHolder.reset(i4, 0, 0);
        }
        correctOffsets(i4, 1, mediaSourceHolder.timeline.getWindowCount(), mediaSourceHolder.timeline.getPeriodCount());
    ...
    private void addMediaSourcesInternal(int i4, Collection<MediaSourceHolder> collection) {
        for (MediaSourceHolder addMediaSourceInternal : collection) {
            addMediaSourceInternal(i4, addMediaSourceInternal);
            i4++;
        }
    }
    ...
    private void correctOffsets(int i4, int i5, int i6, int i7) {
        ...
        while (i4 < this.mediaSourceHolders.size()) {
            this.mediaSourceHolders.get(i4).childIndex += i5;
            this.mediaSourceHolders.get(i4).firstWindowIndexInChild += i6;
            this.mediaSourceHolders.get(i4).firstPeriodIndexInChild += i7;
            i4++;
        }
    }

    private int findMediaSourceHolderByPeriodIndex(int i4) {
        ...
        while (binarySearch < this.mediaSourceHolders.size() - 1) {
            ...
            if (this.mediaSourceHolders.get(i5).firstPeriodIndexInChild != i4) {
                break;
            }
            binarySearch = i5;
        }
    ...
    private void moveMediaSourceInternal(int i4, int i5) {
        ...
        int i7 = this.mediaSourceHolders.get(min).firstPeriodIndexInChild;
        List<MediaSourceHolder> list = this.mediaSourceHolders;
        list.add(i5, list.remove(i4));
        while (min <= max) {
            MediaSourceHolder mediaSourceHolder = this.mediaSourceHolders.get(min);
            mediaSourceHolder.firstWindowIndexInChild = i6;
            mediaSourceHolder.firstPeriodIndexInChild = i7;
            i6 += mediaSourceHolder.timeline.getWindowCount();
            i7 += mediaSourceHolder.timeline.getPeriodCount();
            min++;
        }
    }
    ...
    private void removeMediaSourceInternal(int i4) {
        ...
        correctOffsets(i4, -1, -deferredTimeline.getWindowCount(), -deferredTimeline.getPeriodCount());
        ...
        if (mediaSourceHolder != null) {
            ...
            if (deferredTimeline.getTimeline() != timeline) {
                ...
                if (!mediaSourceHolder.isPrepared && !timeline.isEmpty()) {
                    ...
                    for (int i4 = 0; i4 < mediaSourceHolder.activeMediaPeriods.size(); i4++) {
                        DeferredMediaPeriod deferredMediaPeriod = mediaSourceHolder.activeMediaPeriods.get(i4);
        ...
        switch (i4) {
            ...
            case 6:
                ...
                for (int i5 = 0; i5 < list.size(); i5++) {
                    ((EventDispatcher) list.get(i5)).dispatchEvent();
                }
                return;
            default:
    ...
    public final synchronized void moveMediaSource(int i4, int i5) {
        moveMediaSource(i4, i5, (Runnable) null);
    }
    ...
    public final synchronized void removeMediaSource(int i4) {
        removeMediaSource(i4, (Runnable) null);
    }
    ...
    public MediaSource.MediaPeriodId getMediaPeriodIdForChildMediaPeriodId(MediaSourceHolder mediaSourceHolder, MediaSource.MediaPeriodId mediaPeriodId) {
        for (int i4 = 0; i4 < mediaSourceHolder.activeMediaPeriods.size(); i4++) {
            if (mediaSourceHolder.activeMediaPeriods.get(i4).id.windowSequenceNumber == mediaPeriodId.windowSequenceNumber) {
    ...
    /* access modifiers changed from: protected */
    public int getWindowIndexForChildWindowIndex(MediaSourceHolder mediaSourceHolder, int i4) {
        return i4 + mediaSourceHolder.firstWindowIndexInChild;
    }
    ...
    public final synchronized void removeMediaSource(int i4, @Nullable Runnable runnable) {
        try {
            this.mediaSourcesPublic.remove(i4);
            ExoPlayer exoPlayer = this.player;
            if (exoPlayer != null) {
                exoPlayer.createMessage(this).setType(2).setPayload(new MessageData(i4, null, runnable)).send();
            } else if (runnable != null) {
                runnable.run();
            }
        } catch (Throwable th) {
    ...
    public final synchronized void addMediaSource(int i4, MediaSource mediaSource, @Nullable Runnable runnable) {
        try {
            Assertions.checkNotNull(mediaSource);
            MediaSourceHolder mediaSourceHolder = new MediaSourceHolder(mediaSource);
            this.mediaSourcesPublic.add(i4, mediaSourceHolder);
            ExoPlayer exoPlayer = this.player;
            if (exoPlayer != null) {
                exoPlayer.createMessage(this).setType(0).setPayload(new MessageData(i4, mediaSourceHolder, runnable)).send();
            } else if (runnable != null) {
                runnable.run();
            }
        } catch (Throwable th) {
    ...
    public final synchronized void addMediaSources(int i4, Collection<MediaSource> collection, @Nullable Runnable runnable) {
        try {
            ...
            if (this.player != null && !collection.isEmpty()) {
                this.player.createMessage(this).setType(1).setPayload(new MessageData(i4, arrayList, runnable)).send();
            } else if (runnable != null) {
                runnable.run();
            }
        } catch (Throwable th) {
