...
public class DefaultDrmSessionManager<T extends ExoMediaCrypto> implements DefaultDrmSession.ProvisioningManager<T>, DrmSessionManager<T> {
    ...
    private class MediaDrmEventListener implements ExoMediaDrm.OnEventListener<T> {
        ...
        public void onEvent(ExoMediaDrm<? extends T> exoMediaDrm, byte[] bArr, int i4, int i5, byte[] bArr2) {
            if (DefaultDrmSessionManager.this.mode == 0) {
                DefaultDrmSessionManager.this.mediaDrmHandler.obtainMessage(i4, bArr).sendToTarget();
            }
        }
    }
        ...
        if (C.WIDEVINE_UUID.equals(uuid2)) {
            for (int i6 = 0; i6 < arrayList.size(); i6++) {
                ...
                if (schemeData2.hasData()) {
                    i4 = PsshAtomUtil.parseVersion(schemeData2.data);
                } else {
                    i4 = -1;
                }
                int i7 = Util.SDK_INT;
                if ((i7 < 23 && i4 == 0) || (i7 >= 23 && i4 == 1)) {
                    return schemeData2;
                }
            }
        }
    ...
    @Deprecated
    public DefaultDrmSessionManager(UUID uuid2, ExoMediaDrm<T> exoMediaDrm, MediaDrmCallback mediaDrmCallback, HashMap<String, String> hashMap, Handler handler, DefaultDrmSessionEventListener defaultDrmSessionEventListener, boolean z4, int i4) {
        this(uuid2, exoMediaDrm, mediaDrmCallback, hashMap, z4, i4);
        if (handler != null && defaultDrmSessionEventListener != null) {
            addListener(handler, defaultDrmSessionEventListener);
        }
    }

    public DefaultDrmSessionManager(UUID uuid2, ExoMediaDrm<T> exoMediaDrm, MediaDrmCallback mediaDrmCallback, HashMap<String, String> hashMap, boolean z4, int i4) {
        ...
        this.initialDrmRequestRetryCount = i4;
        this.mode = 0;
