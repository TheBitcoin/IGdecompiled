...
class DefaultDrmSession<T extends ExoMediaCrypto> implements DrmSession<T> {
    ...
    private class PostRequestHandler extends Handler {
        ...
        private long getRetryDelayMillis(int i4) {
            return (long) Math.min((i4 - 1) * 1000, DefaultLoadControl.DEFAULT_BUFFER_FOR_PLAYBACK_AFTER_REBUFFER_MS);
        }

        private boolean maybeRetryRequest(Message message) {
            int i4;
            if (message.arg1 != 1 || (i4 = message.arg2 + 1) > DefaultDrmSession.this.initialDrmRequestRetryCount) {
                return false;
            }
            Message obtain = Message.obtain(message);
            obtain.arg2 = i4;
            sendMessageDelayed(obtain, getRetryDelayMillis(i4));
            return true;
        }
    ...
    public void onKeyResponse(Object obj, Object obj2) {
        if (obj == this.currentKeyRequest && isOpen()) {
            ...
            try {
                ...
                int i4 = this.mode;
                if (!((i4 != 2 && (i4 != 0 || this.offlineLicenseKeySetId == null)) || provideKeyResponse == null || provideKeyResponse.length == 0)) {
    ...
    private void postKeyRequest(int i4, boolean z4) {
        byte[] bArr;
        String str;
        byte[] bArr2;
        String str2;
        ...
        try {
            Pair create = Pair.create(this.mediaDrm.getKeyRequest(bArr3, bArr2, str, i4, this.optionalKeyRequestParameters), str2);
    ...
    public void acquire() {
        int i4 = this.openCount + 1;
        this.openCount = i4;
        if (i4 == 1 && this.state != 1 && openInternal(true)) {
            doLicense(true);
        }
    }
