...
public final class LatmReader implements ElementaryStreamReader {
    ...
    private void parsePayloadMux(ParsableBitArray parsableBitArray, int i4) {
        int position = parsableBitArray.getPosition();
        if ((position & 7) == 0) {
            this.sampleDataBuffer.setPosition(position >> 3);
        } else {
            parsableBitArray.readBits(this.sampleDataBuffer.data, 0, i4 * 8);
            this.sampleDataBuffer.setPosition(0);
        }
        this.output.sampleData(this.sampleDataBuffer, i4);
        this.output.sampleMetadata(this.timeUs, 1, i4, 0, (TrackOutput.CryptoData) null);
    ...
    private void parseStreamMuxConfig(ParsableBitArray parsableBitArray) throws ParserException {
        ...
        if (readBits == 1) {
            i4 = parsableBitArray2.readBits(1);
        } else {
            i4 = 0;
        }
        this.audioMuxVersionA = i4;
        if (i4 == 0) {
            if (readBits == 1) {
