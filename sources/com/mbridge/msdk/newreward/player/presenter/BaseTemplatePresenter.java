...
public abstract class BaseTemplatePresenter extends AbsPresenter implements VideoPlayerStatusListener {
    ...
    private void checkOMSdkProgress(int i4, int i5) {
        try {
            if (this.playTempleModel != null) {
                if (this.omsdkPercentageMap.isEmpty()) {
                    for (int i6 = 0; i6 <= 100; i6 += 25) {
                        this.omsdkPercentageMap.put(Integer.valueOf((i5 * i6) / 100), Integer.valueOf(i6));
                    }
                }
                if (i4 == 1 && this.omsdkPercentageMap.contains(0)) {
                    i4 = 0;
                }
                if (this.omsdkPercentageMap.containsKey(Integer.valueOf(i4))) {
                    int intValue = this.omsdkPercentageMap.get(Integer.valueOf(i4)).intValue();
                    this.omsdkPercentageMap.remove(Integer.valueOf(i4));
                    this.playTempleModel.checkOMSdkProgress(intValue, i5);
                }
            }
        } catch (Exception e5) {
            if (MBridgeConstans.DEBUG) {
    ...
    private void checkPlayCloseBtnState(int i4, int i5) {
        int i6;
        int i7;
        int i8;
        if (i4 == i5) {
        ...
        if (!this.adapterModel.ab() && (i8 = this.videoCompleteTime) > 0 && i4 > i8) {
            this.adapterModel.m(true);
        }
        if ((!this.closeBtnVisible && (i7 = this.videoSkipTime) >= 0 && i4 >= i7) || ((i6 = this.videoCompleteTime) > 0 && i4 > i6)) {
            this.closeBtnVisible = true;
            this.iView.setAdCloseVisibility(true);
        }
    }

    private void checkTrackingForPlayPercentage(int i4, int i5) {
            ...
            if (!this.hasIteratorPercentage && (concurrentHashMap2 = this.percentageMap) != null && concurrentHashMap2.isEmpty()) {
                ...
                for (Map<Integer, String> entrySet : this.campaignEx.getNativeVideoTracking().i()) {
                    for (Map.Entry entry : entrySet.entrySet()) {
                        this.percentageMap.put(Integer.valueOf((int) ((((float) ((Integer) entry.getKey()).intValue()) / 100.0f) * ((float) i5))), entry.getKey());
                    }
                }
            }
            if (i4 == 1 && (concurrentHashMap = this.percentageMap) != null && concurrentHashMap.containsKey(0)) {
                i4 = 0;
            }
            ConcurrentHashMap<Integer, Integer> concurrentHashMap3 = this.percentageMap;
            if (concurrentHashMap3 != null && concurrentHashMap3.containsKey(Integer.valueOf(i4))) {
                int intValue = this.percentageMap.get(Integer.valueOf(i4)).intValue();
                this.percentageMap.remove(Integer.valueOf(i4));
    ...
    private void setCountDownInfo(int i4, int i5) {
        try {
            c cVar = this.commandManager;
            Object d5 = cVar.d((Object) cVar.a("campaign", this.campaignEx, "isIV", Boolean.valueOf(this.isIV), "videoCompleteTime", Integer.valueOf(this.videoCompleteTime), "videoSkipTime", Integer.valueOf(this.videoSkipTime), "curPlayPosition", Integer.valueOf(i4), "allDuration", Integer.valueOf(i5)), f.TEMP_PLAY_PROGRESS_COUNT_DOWN);
            ...
            if (iPlayTempleView != null && iPlayTempleView.getPrivacyButton() != null) {
                this.iView.getPrivacyButton().setTag(String.valueOf(i4));
            }
        } catch (Exception e5) {
            af.b("BaseTemplatePresenter", e5.getMessage());
        }
    ...
    private void setSkipAndCompleteTime(int i4) {
    ...
    private void showIVRewardDialogOnPlayMode() {
        if (isIVRewardEnable() && this.ivRewardAlertViewStatus == a.f13108F && this.ivRewardMode == a.f13111I) {
            int i4 = this.ivRewardValueType;
            if (i4 == a.f13112J && ((int) ((((float) this.currentPlayProgress) / ((float) this.videoTotalLength)) * 100.0f)) >= this.ivRewardValue) {
                onPause();
                this.ivRewardAlertViewStatus = a.f13109G;
                this.iView.setAlertViewType(this.adapterModel.H(), 0, this.ivRewardMode);
                this.iView.alertShow();
            } else if (i4 == a.f13113K && this.currentPlayProgress >= this.ivRewardValue) {
                onPause();
                this.ivRewardAlertViewStatus = a.f13109G;
                this.iView.setAlertViewType(this.adapterModel.H(), 0, this.ivRewardMode);
                this.iView.alertShow();
    ...
    private void updateSegmentsProgressBar(int i4, int i5) {
        try {
            if (this.iView.getSegmentsProgressBar() != null && this.iView.getSegmentsProgressBar().getVisibility() == 0) {
                this.iView.getSegmentsProgressBar().setProgress((i4 * 100) / i5, 0);
            }
        } catch (Exception e5) {
            af.b("BaseTemplatePresenter", e5.getMessage());
        }
    ...
    public void adShowFail(String str, int i4) {
        if (!this.adapterModel.Y()) {
            this.adapterModel.j(true);
            super.adShowFail(str, i4);
            IPlayModel iPlayModel = this.playTempleModel;
            if (iPlayModel != null) {
                iPlayModel.onShowFail(this.mBridgeIds, str, i4);
            }
            finish(true);
        }
    }
    ...
    public void click(View view) {
        if (view.getId() == filterFindViewId(false, "mbridge_alertview_close_button")) {
            if (isIVRewardEnable()) {
                this.ivRewardAlertViewStatus = a.f13106D;
            }
            ...
            if (isIVRewardEnable()) {
                this.ivRewardAlertViewStatus = a.f13107E;
    ...
    public void onPlayProgress(int i4, int i5) {
        ...
        if (this.iView != null && this.campaignEx != null) {
            try {
                this.currentPlayProgress = i4;
                if (i5 == 0) {
                    i6 = 1;
                } else {
                    i6 = i5;
                }
                this.videoTotalLength = i6;
                adShowSuccess();
                advImpList(i4);
                setCountDownInfo(i4, i5);
                if (isIVRewardEnable()) {
                    showIVRewardDialogOnPlayMode();
                }
                updateSegmentsProgressBar(i4, i5);
                checkOMSdkProgress(i4, i5);
                checkPlayCloseBtnState(i4, i5);
                checkTrackingForPlayPercentage(this.currentPlayProgress, i5);
            } catch (Exception e5) {
                af.b("BaseTemplatePresenter", e5.getMessage());
            }
        }
    ...
    public void showIVRewardDialogOnCloseMode() {
        if (!isIVRewardEnable()) {
            closeAdOrShowEndCard();
        } else if (this.ivRewardMode == a.f13110H) {
            int i4 = this.ivRewardValueType;
            if (i4 == a.f13112J && ((int) ((((float) this.currentPlayProgress) / ((float) this.videoTotalLength)) * 100.0f)) >= this.ivRewardValue) {
                closeAdOrShowEndCard();
            } else if (i4 != a.f13113K || this.currentPlayProgress < this.ivRewardValue) {
