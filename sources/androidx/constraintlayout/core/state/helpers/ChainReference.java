...
public class ChainReference extends HelperReference {
    ...
    public void addChainElement(@NonNull String str, float f4, float f5, float f6) {
        addChainElement(str, f4, f5, f6, 0.0f, 0.0f);
    }
    ...
    public void addChainElement(@NonNull Object obj, float f4, float f5, float f6, float f7, float f8) {
        super.add(obj);
        ...
        if (!Float.isNaN(f5)) {
            this.mMapPreMargin.put(obj2, Float.valueOf(f5));
        }
        if (!Float.isNaN(f6)) {
            this.mMapPostMargin.put(obj2, Float.valueOf(f6));
        }
        if (!Float.isNaN(f7)) {
            if (this.mMapPreGoneMargin == null) {
                this.mMapPreGoneMargin = new HashMap<>();
            }
            this.mMapPreGoneMargin.put(obj2, Float.valueOf(f7));
        }
        if (!Float.isNaN(f8)) {
            if (this.mMapPostGoneMargin == null) {
                this.mMapPostGoneMargin = new HashMap<>();
            }
            this.mMapPostGoneMargin.put(obj2, Float.valueOf(f8));
        }
    }
