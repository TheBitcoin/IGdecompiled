...
public class RowHeaderPresenter extends Presenter {
    ...
    public void onSelectLevelChanged(ViewHolder viewHolder) {
        if (this.mAnimateSelect) {
            View view = viewHolder.view;
            float f4 = viewHolder.mUnselectAlpha;
            view.setAlpha(f4 + (viewHolder.mSelectLevel * (1.0f - f4)));
        }
    }
    ...
    public final void setSelectLevel(ViewHolder viewHolder, float f4) {
        viewHolder.mSelectLevel = f4;
        onSelectLevelChanged(viewHolder);
    }

    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public RowHeaderPresenter(int i4) {
        this(i4, true);
    }

    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public RowHeaderPresenter(int i4, boolean z4) {
