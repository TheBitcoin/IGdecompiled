...
public class CompositeDrawable extends Drawable implements Drawable.Callback {
    ...
    public void draw(Canvas canvas) {
        ...
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            arrayList.get(i4).mDrawable.draw(canvas);
        }
    }
    ...
    public ChildDrawable getChildAt(int i4) {
        return this.mState.mChildren.get(i4);
    }
    ...
    public Drawable getDrawable(int i4) {
        return this.mState.mChildren.get(i4).mDrawable;
    }
    ...
    public final Drawable getFirstNonNullDrawable() {
        ...
        for (int i4 = 0; i4 < size; i4++) {
            Drawable drawable = arrayList.get(i4).mDrawable;
            if (drawable != null) {
                return drawable;
            }
        }
    ...
    public void removeChild(int i4) {
        this.mState.mChildren.remove(i4);
    }

    public void removeDrawable(Drawable drawable) {
        ArrayList<ChildDrawable> arrayList = this.mState.mChildren;
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            if (drawable == arrayList.get(i4).mDrawable) {
                arrayList.get(i4).mDrawable.setCallback((Drawable.Callback) null);
                arrayList.remove(i4);
                return;
            }
        }
    ...
    public void setAlpha(int i4) {
        ...
        for (int i5 = 0; i5 < arrayList.size(); i5++) {
            arrayList.get(i5).mDrawable.setAlpha(i4);
        }
    }

    public void setChildDrawableAt(int i4, Drawable drawable) {
        this.mState.mChildren.set(i4, new ChildDrawable(drawable, this));
    }

    public void setColorFilter(ColorFilter colorFilter) {
        ArrayList<ChildDrawable> arrayList = this.mState.mChildren;
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            arrayList.get(i4).mDrawable.setColorFilter(colorFilter);
        }
    }
    ...
    public void updateBounds(Rect rect) {
        ArrayList<ChildDrawable> arrayList = this.mState.mChildren;
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            arrayList.get(i4).updateBounds(rect);
        }
    }
    ...
    static class CompositeState extends Drawable.ConstantState {
        ...
        CompositeState(CompositeState compositeState, CompositeDrawable compositeDrawable, Resources resources) {
            ...
            for (int i4 = 0; i4 < size; i4++) {
                this.mChildren.add(new ChildDrawable(compositeState.mChildren.get(i4), compositeDrawable, resources));
            }
        }
    }
    ...
    public static final class ChildDrawable {
        ...
        static {
            ...
            TOP_FRACTION = new Property<ChildDrawable, Float>(cls2, "fractionTop") {
                ...
                public void set(ChildDrawable childDrawable, Float f4) {
                    if (childDrawable.getBoundsRule().top == null) {
                        childDrawable.getBoundsRule().top = BoundsRule.ValueRule.inheritFromParent(f4.floatValue());
                    } else {
                        childDrawable.getBoundsRule().top.setFraction(f4.floatValue());
                    }
                    childDrawable.recomputeBounds();
                }
            };
            BOTTOM_FRACTION = new Property<ChildDrawable, Float>(cls2, "fractionBottom") {
                ...
                public void set(ChildDrawable childDrawable, Float f4) {
                    if (childDrawable.getBoundsRule().bottom == null) {
                        childDrawable.getBoundsRule().bottom = BoundsRule.ValueRule.inheritFromParent(f4.floatValue());
                    } else {
                        childDrawable.getBoundsRule().bottom.setFraction(f4.floatValue());
                    }
                    childDrawable.recomputeBounds();
                }
            };
            LEFT_FRACTION = new Property<ChildDrawable, Float>(cls2, "fractionLeft") {
                ...
                public void set(ChildDrawable childDrawable, Float f4) {
                    if (childDrawable.getBoundsRule().left == null) {
                        childDrawable.getBoundsRule().left = BoundsRule.ValueRule.inheritFromParent(f4.floatValue());
                    } else {
                        childDrawable.getBoundsRule().left.setFraction(f4.floatValue());
                    }
                    childDrawable.recomputeBounds();
                }
            };
            RIGHT_FRACTION = new Property<ChildDrawable, Float>(cls2, "fractionRight") {
                ...
                public void set(ChildDrawable childDrawable, Float f4) {
                    if (childDrawable.getBoundsRule().right == null) {
                        childDrawable.getBoundsRule().right = BoundsRule.ValueRule.inheritFromParent(f4.floatValue());
                    } else {
                        childDrawable.getBoundsRule().right.setFraction(f4.floatValue());
                    }
                    childDrawable.recomputeBounds();
                }
            };
