...
public class ProfileInstallReceiver extends BroadcastReceiver {
    ...
    class ResultDiagnostics implements ProfileInstaller.DiagnosticsCallback {
        ...
        public void onDiagnosticReceived(int i4, @Nullable Object obj) {
            ProfileInstaller.LOG_DIAGNOSTICS.onDiagnosticReceived(i4, obj);
        }

        public void onResultReceived(int i4, @Nullable Object obj) {
            ProfileInstaller.LOG_DIAGNOSTICS.onResultReceived(i4, obj);
    ...
    public void onReceive(@NonNull Context context, @Nullable Intent intent) {
        ...
        if (intent != null) {
            ...
            if (ACTION_INSTALL_PROFILE.equals(action)) {
                ProfileInstaller.writeProfile(context, new h(), new ResultDiagnostics(), true);
            } else if (ACTION_SKIP_FILE.equals(action)) {
                Bundle extras2 = intent.getExtras();
                if (extras2 != null) {
                    String string = extras2.getString(EXTRA_SKIP_FILE_OPERATION);
                    if (EXTRA_SKIP_FILE_OPERATION_WRITE.equals(string)) {
                        ProfileInstaller.writeSkipFile(context, new h(), new ResultDiagnostics());
                    } else if (EXTRA_SKIP_FILE_OPERATION_DELETE.equals(string)) {
                        ProfileInstaller.deleteSkipFile(context, new h(), new ResultDiagnostics());
                    }
                }
