...
public class DifferentialMotionFlingController {
    ...
    interface DifferentialVelocityProvider {
        float getCurrentVelocity(VelocityTracker velocityTracker, MotionEvent motionEvent, int i4);
    }
    ...
    public DifferentialMotionFlingController(@NonNull Context context, @NonNull DifferentialMotionFlingTarget differentialMotionFlingTarget) {
        this(context, differentialMotionFlingTarget, new C0467q(), new r());
    }

    private boolean calculateFlingVelocityThresholds(MotionEvent motionEvent, int i4) {
        int source = motionEvent.getSource();
        int deviceId = motionEvent.getDeviceId();
        if (this.mLastProcessedSource == source && this.mLastProcessedDeviceId == deviceId && this.mLastProcessedAxis == i4) {
            return false;
        }
        this.mVelocityThresholdCalculator.calculateFlingVelocityThresholds(this.mContext, this.mFlingVelocityThresholds, motionEvent, i4);
        this.mLastProcessedSource = source;
        this.mLastProcessedDeviceId = deviceId;
        this.mLastProcessedAxis = i4;
        return true;
    }

    private float getCurrentVelocity(MotionEvent motionEvent, int i4) {
        if (this.mVelocityTracker == null) {
            this.mVelocityTracker = VelocityTracker.obtain();
        }
        return this.mVelocityProvider.getCurrentVelocity(this.mVelocityTracker, motionEvent, i4);
    }

    public void onMotionEvent(@NonNull MotionEvent motionEvent, int i4) {
        boolean calculateFlingVelocityThresholds = calculateFlingVelocityThresholds(motionEvent, i4);
        ...
        float currentVelocity = getCurrentVelocity(motionEvent, i4) * this.mTarget.getScaledScrollFactor();
        ...
        if (abs >= ((float) iArr[0])) {
            int i5 = iArr[1];
            float max = Math.max((float) (-i5), Math.min(currentVelocity, (float) i5));
            if (this.mTarget.startDifferentialMotionFling(max)) {
                f4 = max;
            }
            this.mLastFlingVelocity = f4;
        }
    }
    ...
    public static float getCurrentVelocity(VelocityTracker velocityTracker, MotionEvent motionEvent, int i4) {
