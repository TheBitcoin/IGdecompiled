...
public final class WindowMetricsCalculatorCompat implements WindowMetricsCalculator {
    ...
    public final Rect computeWindowBoundsIceCreamSandwich$window_release(Activity activity) {
        ...
        Point realSizeForDisplay$window_release = getRealSizeForDisplay$window_release(defaultDisplay);
        Rect rect = new Rect();
        int i5 = realSizeForDisplay$window_release.x;
        if (i5 == 0 || (i4 = realSizeForDisplay$window_release.y) == 0) {
    ...
    public final Rect computeWindowBoundsN$window_release(Activity activity) {
        ...
        if (!ActivityCompatHelperApi24.INSTANCE.isInMultiWindowMode(activity)) {
            ...
            int i4 = rect.bottom;
            if (i4 + navigationBarHeight == realSizeForDisplay$window_release.y) {
                rect.bottom = i4 + navigationBarHeight;
                return rect;
            }
            int i5 = rect.right;
            if (i5 + navigationBarHeight == realSizeForDisplay$window_release.x) {
                rect.right = i5 + navigationBarHeight;
            }
        }
        return rect;
    }
    ...
    public final Rect computeWindowBoundsP$window_release(Activity activity) {
        ...
        if (!activityCompatHelperApi24.isInMultiWindowMode(activity)) {
            int navigationBarHeight = getNavigationBarHeight(activity);
            int i4 = rect.bottom;
            if (i4 + navigationBarHeight == point.y) {
                rect.bottom = i4 + navigationBarHeight;
            } else {
                int i5 = rect.right;
                if (i5 + navigationBarHeight == point.x) {
                    rect.right = i5 + navigationBarHeight;
