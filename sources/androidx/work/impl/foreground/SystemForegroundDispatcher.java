...
public class SystemForegroundDispatcher implements OnConstraintsStateChangedListener, ExecutionListener {
    ...
    final Map<WorkGenerationalId, C1001s0> mTrackedWorkSpecs;
    /* access modifiers changed from: private */
    ...
    interface Callback {
        ...
        void notify(int i4, @NonNull Notification notification);

        void startForeground(int i4, int i5, @NonNull Notification notification);
        ...
        if (notification != null && this.mCallback != null) {
            ...
            if (intExtra2 != 0 && Build.VERSION.SDK_INT >= 29) {
                for (Map.Entry<WorkGenerationalId, ForegroundInfo> value : this.mForegroundInfoById.entrySet()) {
                    i4 |= ((ForegroundInfo) value.getValue()).getForegroundServiceType();
                }
                ForegroundInfo foregroundInfo = this.mForegroundInfoById.get(this.mCurrentForegroundId);
                if (foregroundInfo != null) {
                    this.mCallback.startForeground(foregroundInfo.getNotificationId(), i4, foregroundInfo.getNotification());
                }
            }
        }
    }
    ...
    public void onDestroy() {
        ...
        synchronized (this.mLock) {
            try {
                for (C1001s0 a5 : this.mTrackedWorkSpecs.values()) {
        ...
        synchronized (this.mLock) {
            try {
                if (this.mWorkSpecById.remove(workGenerationalId) != null) {
                    s0Var = this.mTrackedWorkSpecs.remove(workGenerationalId);
                } else {
                    s0Var = null;
                }
                if (s0Var != null) {
