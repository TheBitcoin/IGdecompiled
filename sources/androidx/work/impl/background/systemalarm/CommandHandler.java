...
public class CommandHandler implements ExecutionListener {
    ...
    static Intent createExecutionCompletedIntent(@NonNull Context context, @NonNull WorkGenerationalId workGenerationalId, boolean z4) {
        Intent intent = new Intent(context, SystemAlarmService.class);
    ...
    private void handleConstraintsChanged(@NonNull Intent intent, int i4, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        Logger logger = Logger.get();
        String str = TAG;
        logger.debug(str, "Handling constraints changed " + intent);
        new ConstraintsCommandHandler(this.mContext, this.mClock, i4, systemAlarmDispatcher).handleConstraintsChanged();
    }

    private void handleDelayMet(@NonNull Intent intent, int i4, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        synchronized (this.mLock) {
            try {
                ...
                if (!this.mPendingDelayMet.containsKey(readWorkGenerationalId)) {
                    DelayMetCommandHandler delayMetCommandHandler = new DelayMetCommandHandler(this.mContext, i4, systemAlarmDispatcher, this.mStartStopTokens.tokenFor(readWorkGenerationalId));
    ...
    private void handleExecutionCompleted(@NonNull Intent intent, int i4) {
        WorkGenerationalId readWorkGenerationalId = readWorkGenerationalId(intent);
        boolean z4 = intent.getExtras().getBoolean(KEY_NEEDS_RESCHEDULE);
        Logger logger = Logger.get();
        String str = TAG;
        logger.debug(str, "Handling onExecutionCompleted " + intent + ", " + i4);
        onExecuted(readWorkGenerationalId, z4);
    }

    private void handleReschedule(@NonNull Intent intent, int i4, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        Logger logger = Logger.get();
        String str = TAG;
        logger.debug(str, "Handling reschedule " + intent + ", " + i4);
        systemAlarmDispatcher.getWorkManager().rescheduleEligibleWork();
    }

    private void handleScheduleWorkIntent(@NonNull Intent intent, int i4, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        WorkGenerationalId readWorkGenerationalId = readWorkGenerationalId(intent);
        Logger logger = Logger.get();
        String str = TAG;
        ...
        try {
            ...
            if (workSpec == null) {
                ...
                if (!workSpec.hasConstraints()) {
                    ...
                    Alarms.setAlarm(this.mContext, workDatabase, readWorkGenerationalId, calculateNextRunTime);
                    systemAlarmDispatcher.getTaskExecutor().getMainThreadExecutor().execute(new SystemAlarmDispatcher.AddRunnable(systemAlarmDispatcher, createConstraintsChangedIntent(this.mContext), i4));
                }
    ...
    private void handleStopWork(@NonNull Intent intent, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        ...
        if (extras.containsKey(KEY_WORKSPEC_GENERATION)) {
            ...
            StartStopToken remove = this.mStartStopTokens.remove(new WorkGenerationalId(string, i4));
            if (remove != null) {
                list.add(remove);
            }
        } else {
    ...
    /* access modifiers changed from: package-private */
    @WorkerThread
    public void onHandleIntent(@NonNull Intent intent, int i4, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        String action = intent.getAction();
        if (ACTION_CONSTRAINTS_CHANGED.equals(action)) {
            handleConstraintsChanged(intent, i4, systemAlarmDispatcher);
        } else if (ACTION_RESCHEDULE.equals(action)) {
            handleReschedule(intent, i4, systemAlarmDispatcher);
            ...
            handleScheduleWorkIntent(intent, i4, systemAlarmDispatcher);
        } else if (ACTION_DELAY_MET.equals(action)) {
            handleDelayMet(intent, i4, systemAlarmDispatcher);
        } else if (ACTION_STOP_WORK.equals(action)) {
            handleStopWork(intent, systemAlarmDispatcher);
        } else if (ACTION_EXECUTION_COMPLETED.equals(action)) {
            handleExecutionCompleted(intent, i4);
        } else {
            Logger logger2 = Logger.get();
            String str2 = TAG;
