...
public abstract class TrustedWebActivityService extends Service {
            ...
            if (trustedWebActivityService.mVerifiedUid == -1) {
                ...
                if (load != null) {
                    ...
                    while (true) {
                        if (i4 >= length) {
                            break;
                        } else if (load.matches(packagesForUid[i4], packageManager)) {
    ...
    @BinderThread
    public void onCancelNotification(@NonNull String str, int i4) {
        ensureOnCreateCalled();
    ...
    @RequiresPermission("android.permission.POST_NOTIFICATIONS")
    @BinderThread
    public boolean onNotifyNotificationWithChannel(@NonNull String str, int i4, @NonNull Notification notification, @NonNull String str2) {
        ensureOnCreateCalled();
        if (!NotificationManagerCompat.from(this).areNotificationsEnabled()) {
            return false;
        }
