...
public class BrowserActionsIntent {
    ...
    public static List<BrowserActionItem> parseBrowserActionItems(@NonNull ArrayList<Bundle> arrayList) {
        ...
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            ...
            int i5 = bundle.getInt(KEY_ICON_ID);
            Uri uri = (Uri) bundle.getParcelable(KEY_ICON_URI);
            if (TextUtils.isEmpty(string) || pendingIntent == null) {
                throw new IllegalArgumentException("Custom item should contain a non-empty title and non-null intent.");
            }
            if (i5 != 0) {
                browserActionItem = new BrowserActionItem(string, pendingIntent, i5);
            } else {
        ...
        if (list.size() == 1) {
            ...
            if (resolveActivity != null) {
                ...
                while (true) {
                    if (i4 >= list.size()) {
                        break;
                    } else if (str.equals(list.get(i4).activityInfo.packageName)) {
    ...
    public static void openBrowserAction(@NonNull Context context, @NonNull Uri uri, int i4, @NonNull ArrayList<BrowserActionItem> arrayList, @NonNull PendingIntent pendingIntent) {
        launchIntent(context, new Builder(context, uri).setUrlType(i4).setCustomItems(arrayList).setOnItemSelectedAction(pendingIntent).build().getIntent());
    }
    ...
    public static final class Builder {
        ...
        public Builder setCustomItems(@NonNull ArrayList<BrowserActionItem> arrayList) {
            if (arrayList.size() <= 5) {
                for (int i4 = 0; i4 < arrayList.size(); i4++) {
                    if (TextUtils.isEmpty(arrayList.get(i4).getTitle()) || arrayList.get(i4).getAction() == null) {
                        throw new IllegalArgumentException("Custom item should contain a non-empty title and non-null intent.");
                    }
                    this.mMenuItems.add(getBundleFromItem(arrayList.get(i4)));
                    if (arrayList.get(i4).getIconUri() != null) {
                        this.mImageUris.add(arrayList.get(i4).getIconUri());
                    }
                }
                return this;
            }
        ...
        @NonNull
        public Builder setUrlType(int i4) {
            this.mType = i4;
            return this;
        }
